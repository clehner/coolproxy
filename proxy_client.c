...
static int http_parser_status_cb(void *client, void *data) {
    return proxy_client_on_http_request((struct proxy_client *)client,
            (struct http_parser_request *)data);
}
...
struct proxy_client *proxy_client_new(eventloop_t loop,
    ...
    client->parser_callbacks.on_request.obj = client;
    client->parser_callbacks.on_request.fn = http_parser_status_cb;
